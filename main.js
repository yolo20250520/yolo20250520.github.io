...
ball_sizes = [1, 1.53, 2.19, 2.87, 3.23, 3.52, 5.57, 8.11]
function new_ball(x, y, rank, group, isStatic = false, vx = 0, vy = 0) {
    ...
    var res = Bodies.circle(x, y, r, {
        ...
        friction: 0.03,
        restitution: 0.1,
        label: { rank: rank, group: group },
        render: {
            sprite: {
                texture: "./img/" + (rank + 10 * group) + ".png",
                xScale: 2 * r / 500,
                yScale: 2 * r / 500
            }
        }
...
function randomRank(group) {
    var n = maxRank[group] + 1;
    var sum = n * (n + 1) / 2;
    ...
    return maxRank[group];
}

function randomGroup() {
    var numerator = largest[1] + 1;
    var denom = largest[0] + largest[1] + 2;
    if (Math.random() > numerator / denom) { return 1; } else { return 0; }
}
...
var ground = Bodies.rectangle(w / 2, 0.95 * h, w * 1.1, h * 0.1, {
...
for (let i = 0; i < 7; i++) {
    World.add(engine.world, new_ball(-w, -h, i, 0, true));
    World.add(engine.world, new_ball(-w, -h, i, 1, true));
}
World.add(engine.world, new_ball(-w, -h, 7, 0, true));
...
Events.on(mouseConstraint, 'mouseup', function (event) {
    var x = event.mouse.position.x,
        ...
        ball = new_ball(x, -1, next.label.rank, next.label.group, false, 0, h / 80);
    console.log(ball);
    ...
    next = new_ball(w / 2, -1, randomRank(group), group, true);
...
Events.on(engine, 'collisionStart', function (event) {
    ...
    for (var i = 0; i < pairs.length; i++) {
        var pair = pairs[i];
        if (pair.bodyA.label.group == pair.bodyB.label.group && pair.bodyA.label.rank == pair.bodyB.label.rank && pair.bodyA.label.rank < 6) {
            ...
            var g = pair.bodyA.label.group
            var ball = new_ball(x, y, pair.bodyA.label.rank + 1, pair.bodyA.label.group, false, vx, vy);
            ...
            maxRank[g] = Math.min(3, Math.max(maxRank[g], ball.label.rank));
            if (ball.label.rank == 6) { largest[g] += 1; }
        }
        if (pair.bodyA.label.group != pair.bodyB.label.group && pair.bodyA.label.rank == pair.bodyB.label.rank && pair.bodyA.label.rank == 6) {
            var x = (pair.bodyA.position.x + pair.bodyB.position.x) * 0.5;
            var y = (pair.bodyA.position.y + pair.bodyB.position.y) * 0.5;
            var vx = (pair.bodyA.velocity.x + pair.bodyB.velocity.x) * 0.5;
            var vy = (pair.bodyA.velocity.y + pair.bodyB.velocity.y) * 0.5;
            var ball = new_ball(x, y, pair.bodyA.label.rank + 1, 0, false, vx, vy);
            // Body.setVelocity(ball, v);
            World.remove(engine.world, [pair.bodyA, pair.bodyB]);
            pair.bodyA.label.rank = -1; // to avoid multiple match for the same ball.
            pair.bodyB.label.rank = -1;
